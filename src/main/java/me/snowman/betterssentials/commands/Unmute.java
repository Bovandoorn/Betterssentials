...
public class Unmute implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!muteManager.getMutes().containsKey(player.getUniqueId())) {
            sender.sendMessage(prefix + messageManager.getMessage("NotMuted"));
            return true;
        }
        muteManager.getMutes().remove(player.getUniqueId());
        player.sendMessage(prefix + messageManager.getMessage("NoLongerMuted"));
        return true;
    }
}
